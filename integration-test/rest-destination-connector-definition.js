...
export function CheckList() {
    ...
    group("Connector API: List destination connector definitions", () => {
        ...
        var limitedRecords = http.request("GET", `${connectorHost}/v1alpha/destination-connector-definitions`)
        ...
        check(http.request("GET", `${connectorHost}/v1alpha/destination-connector-definitions?page_size=${limitedRecords.json().total_size}`), {
            ...
            [`GET /v1alpha/destination-connector-definitions?page_size=${limitedRecords.json().total_size} response next_page_token empty`]: (r) => r.json().next_page_token === "",
        });
    });
}
