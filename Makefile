...
dev:							## Run dev container
	...
	docker run -d --rm -v $(PWD):/${SERVICE_NAME} -v vdp:/vdp -v airbyte:/airbyte -v /var/run/docker.sock:/var/run/docker.sock \
...
integration-test:				## Run integration test
	...
	@bash -c 'if [ "$(HOSTNAME)" == "" ]; then echo "Test destination connector data sanity..." && python3 integration-test/check_connector_destination.py && (echo "Pass" && true) || (echo "Failed" && false); fi'
